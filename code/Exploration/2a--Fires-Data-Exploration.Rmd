...
gdf_fires_area <- fires %>%
  ...
  dplyr::select(!!sym(fire_id_column), total_fire_area_ha)
...
analysis_data1 <- filter(analysis_data, proportion_treated < 0.99)
mean(analysis_data1$proportion_treated)
range(analysis_data1$proportion_treated)

# Create the histogram using ggplot2
ggplot(data = analysis_data, aes(x = proportion_treated)) +
  geom_histogram(bins = 20) +
  labs(
    title = "Distribution of Proportion of Fire Area Treated (2010-2024)",
...
treatment_area_data_by_type <- intersection %>%
  ...
  dplyr::select(MGT_TYPE, treatment_size_ha)
...
elevation_rast <- rast("data/raw/LANDFIRE/LF2020_Elev_220_CONUS/Tif/LC20_Elev_220.tif")
...
fire_names <- unique(fires$Event_ID)
results_list <- list()
...
char_table <- fires %>%
  inner_join(analysis_data, by = "Event_ID") %>%
  inner_join(elev_table, by = "Event_ID")
...
result_table <- char_table %>%
  ...
  dplyr::select(
    Event_ID, Incid_Name, 
    year,
    'Fire_Area' = total_fire_area_ha, 
    'Proportion_Treated' = proportion_treated, 
    Fire_Min_Elev, Fire_Max_Elev,
    Treatment_Min_Elev, Treatment_Max_Elev,
...
write.csv(result_table, "data/processed/my_fires.csv", row.names = FALSE)
```
