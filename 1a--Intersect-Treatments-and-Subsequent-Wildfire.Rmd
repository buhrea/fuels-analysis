...
fires <- st_read("data/mtbs_perimeter_data/mtbs_perims_DD.shp") %>% 
  ...
  filter(year>=2001) %>%  # filter for fires that occurred in or after 2015
...
fires <- fires %>% 
  ...
  dplyr::select("Event_ID", "irwinID", "Incid_Name", "Map_ID", "Ig_Date", "year")
```
...
st_write(
  obj = intersect,
  dsn = "geopackages/fuels_analysis.gpkg",
  layer = "intersection",
  driver = "GPKG",
  delete_dsn = TRUE
)
